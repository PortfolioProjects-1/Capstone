...
x_county <- arima(TN_County_Selection$Population, order=c(1,1,2))
...
ui <- shinyUI(
  ...
  navbarPage("SNF Optimal Locator", theme = shinytheme("flatly"),
             tabPanel("TN State Map", 
                      sidebarLayout(
                        ...
                        mainPanel(tabsetPanel(id = "tabspanel", type = "tabs",
                                              tabPanel(title = "SNF Optimal Location Map",
                                                       div(
                                                         class = "outer",
                                                         tags$head(
                                                           includeCSS("www/styles.css")
                                                         ),
                                                         leafletOutput("population_view",width="100%",height="100px")
                                                       )
                                              ))))),
             tabPanel("TN County Chart", 
                      sidebarLayout(
                        ...
                        mainPanel(tabsetPanel(id = "tabspanel", type = "tabs",
                                              tabPanel(title = "County Population Forecast",
             ...
             tabPanel("TN Tract Chart", 
                      sidebarLayout(
                        ...
                        mainPanel(tabsetPanel(id = "tabspanel", type = "tabs",
                                              tabPanel(title = "Tract Population Forecast",
...
server <- function(input, output) {
  ...
  output$population_view <- renderLeaflet({
    ...
    leaflet(SNF_2015_TN) %>%
      ...
      addTiles() %>%
      addPolygons(data=df_2018.polygon, fillColor = ~pal(Tract_Population_Density_2018),color = "#b2aeae",fillOpacity = 1, 
                  weight = 0.3, 
                  smoothFactor = 0.2) %>%
