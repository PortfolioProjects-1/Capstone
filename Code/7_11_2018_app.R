                         ...
                         tabPanel("TN State Map", 
                                  sidebarLayout(
                                    sidebarPanel(
                                      ...
                                      checkboxInput("plot1","Population Density Map", value=T),
                                      checkboxInput("plot2","Traffic View Map", value=F),
...
server <- function(input, output) {
    ...
    leaflet(SNF_2013_TN) %>%
      ...
      traffic_view <- reactive({
      if(!input$traffic_view) return(NULL)
      leaflet(SNF_2013_TN) %>% 
      setView(-86.5804, 35.5175, zoom = 7) %>%
      addTiles() %>%
      addPolygons(data=df.polygon, fillColor = "transparent",color = "#b2aeae",fillOpacity = 1, 
                  weight = 0.3, 
                  smoothFactor = 0.2) %>%
      addPolygons(data=TN,weight=0.5,col = 'black', fillColor = "Transparent") %>%
      addCircles(lng = SNF_2013_TN$Long,lat = SNF_2013_TN$Lat, color = "red", 
