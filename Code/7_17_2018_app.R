...
shinyApp(
  ui = tagList(
    navbarPage(
      ...
      tabPanel("TN State Map",
               sidebarPanel(
                 h4(paste("Select Map View")),
                 checkboxInput("population_view","Population Density Map", value=T),
                 checkboxInput("traffic_view","Traffic View Map", value=F),
               ...
               mainPanel(
                 tabsetPanel(
                   tabPanel("SNF Optimal Location Map",
                            leafletOutput("population_view")
  ...
  server <- function(input, output) {
    ...
    # tractdata <- reactive({
    #   tractdata <- Tract_Predictions[c("Tract_Name","Target_Demographic_Population_2011", "Target_Demographic_Population_2012",
    ...
    #     filter(Tract_Name == input$County)
    #   
    ...
    output$population_view <- renderLeaflet({
      #if(!input$population_view) return(NULL)
      leaflet(SNF_2015_TN) %>%
        setView(-86.5804, 35.5175, zoom = 7) %>%
        addTiles() %>%
        addPolygons(data=mapdata(), fillColor = ~pal(Tract_Population_Density_2018),color = "#b2aeae",fillOpacity = 1, 
                    weight = 0.3, 
                    smoothFactor = 0.2) %>%
        ...
        addCircles(lng = km$centers[,c(1)], lat = km$centers[,c(2)], color = "blue", 
                   popup = paste("<b>", km$centers[,c(1)], ",",km$centers[,c(2)])) %>%
        addCircles(lng = TN_Tract_Centroids$long, lat = TN_Tract_Centroids$lat, color = "transparent") #%>%
        #addLegend(pal = pal, 
                  #values = mapdata$Tract_Population_Density_2018, 
                  #position = "bottomleft", 
    ...
    output$county_population <- renderHighchart({
       highchart() %>% 
        ...
        hc_add_series(name="Population", data = countydata()) %>%
    ...
    highchart() %>% 
      ...
      hc_xAxis(categories = HC_Year_Series) %>% 
      hc_add_series(name="Population", data = TN_Tract_Selection$Population, color = "red") %>%
      hc_add_theme(hc_theme_db())
